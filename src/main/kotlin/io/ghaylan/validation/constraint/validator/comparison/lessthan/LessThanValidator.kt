...
object LessThanValidator: ConstraintValidator<Comparable<*>, LessThanConstraint>() {
	 ...
	 override fun validate(
		...
		if (isViolation) {
			...
			val code = when (value) {
				is Date,
				is Temporal -> ConstraintErrorCode.INSTANT_TOO_LATE
				is Number -> ConstraintErrorCode.NUMBER_TOO_LARGE
				else -> return null
			}
			
			return ConstraintError(
				code = code,
				message = "The value failed the relative less-than comparison constraint.",
				metadata = buildMap {
					put("ref_name", constraint.property)
